...
import { PubSubChatModActionMessage, PubSubClient, PubSubSubscriptionMessage } from 'twitch-pubsub-client';
...
async function main() {
	...
	const twitchChannels = ['spoodah', 'azfura', 'ilybeamic', 'pacopaco', 'jg7tv', 'sirkenbo'];
	...
	chatClient.onMessage((channel, user, message, msg) => {
		...
		if(message.toLowerCase() === '!prime') {
			publishMessage(channel, `If you have Amazon Prime connect your account here -> twitch.amazon.com/prime ! Once you have linked your accounts click the sub button and sub for FREE PogChamp Need help? Click here -> https://goo.gl/VyzHCX kaleiLove`);
		}
	});

	chatClient.onSub((channel, user, subInfo) => {
		if(subInfo.isPrime) {
			const botMsg = `xqcPrime PRIME SUB!!! xqcPrime @${user} just subscribed using Twitch Prime! Welcome to the squadron kaleiLove`;
			// const chnl = channel.trim();
			// switch (chnl) {
			// 	case '#spoodah':
			// 		let botMsg = `pokiPrime PRIME SUB!!! pokiPrime @${user} just subscribed using Twitch Prime! Welcome to the squadron sirken2LOVE`;
			// 		publishMessage(channel, botMsg);
			// 		break;
			// 	case '#jg7tv':
			...
			// 		break;
			// 	default:
			// 		botMsg = `pokiPrime PRIME SUB!!! pokiPrime @${user} just subscribed using Twitch Prime! Welcome to the squadron sirken2LOVE`;
			// 		publishMessage(channel, botMsg);
			// 		break;
			// }
			publishMessage(channel, botMsg);
			...
			switch(chnl) {
				case '#spoodah':
					let botMsg = `spoodHype NEW SUB!!! spoodHype @${user} just subscribed! Welcome to the squadron kaleiLove`;
					publishMessage(channel, botMsg);
					break;
				// case '#jg7tv':
  ...
  chatClient.onResub((channel, user, subInfo) => {
    if(subInfo.isPrime) {
      const botMsg = `xqcPrime PRIME RESUB!!! xqcPrime Welcome back @${subInfo.displayName} for ${subInfo.months} months kaleiLove`;
      publishMessage(channel, botMsg);
    }
    else if(subInfo.plan === '1000') {
			const botMsg = `spoodHype RESUB!!! spoodHype @${subInfo.displayName} just resubscribed! Welcome back and thanks for the support kaleiLove`;
			publishMessage(channel, botMsg);
    }
    else if(subInfo.plan === '2000') {
			const botMsg = `spoodHype RESUB!!! spoodHype @${subInfo.displayName} just resubscribed at Tier 2 ($9.99 USD)! Welcome back and thanks for the support kaleiLove`;
			publishMessage(channel, botMsg);
    }
    else if(subInfo.plan === '3000') {
			const botMsg = `spoodHype RESUB!!! spoodHype @${subInfo.displayName} just resubscribed at Tier 3 ($24.99 USD)! Welcome back and thanks for the support kaleiLove`;
			publishMessage(channel, botMsg);
		}
  });
  ...
  chatClient.onRaid((channel, user, raidInfo) => {
    const botMsg = `spoodWYA NEW RAID!!! spoodWYA Thank you so much @${raidInfo.displayName} for the ${raidInfo.viewerCount} viewer raid! alexiaWow`;
    publishMessage(channel, botMsg);
  });
  ...
  chatClient.onCommunitySub((channel, user, subInfo, msg) => {
    const previousGiftCount = giftCounts.get(user) ?? 0;
		...
		if (subInfo.count > 1) {
			...
			switch(chnl) {
					case '#spoodah':
						let botMsg = `alexiaGift GIFT SUB HYPE!! alexiaGift Thank you @${subInfo.gifterDisplayName} for gifting ${subInfo.count} subs to the squadron! alexiaLove`;
						publishMessage(channel, botMsg);
						let botMsg2 = `spoodHey Welcome to all the new subscribers! spoodSalute`;
						publishMessage(channel, botMsg2);
						break;
					// case '#jg7tv':
					// 	botMsg = `alexiaGift GIFT SUB HYPE!! alexiaGift Thank you @${subInfo.gifterDisplayName} for gifting ${subInfo.count} new people into the Sub 7's! jg7tvHype`;
					// 	publishMessage(channel, botMsg);
			...
			const botMsg = `alexiaGift RANDOM GIFT SUB HYPE!! alexiaGift Thank you ${subInfo.gifterDisplayName} for gifting a random sub to the chat! alexiaLove`;
      publishMessage(channel, botMsg);
		}
  });
  ...
  chatClient.onSubGift((channel, recipient, subInfo) => {
    ...
    if (previousGiftCount > 0) {
      ...
      const botMsg = `spoodGLB GIFT SUB HYPE!! spoodGLB Thank you ${subInfo.gifterDisplayName} for gifting a sub to ${recipient} alexiaLove`;
      publishMessage(channel, botMsg);
    }
	});
	...
	const modListener = await pubSubClient.onModAction(userId, spoodId, (message: PubSubChatModActionMessage) => {
		if (message.channelId === spoodId.toString()) {
			if (message.action === 'timeout') {
				...
				logChannel.send({ embed: embedVar});
				}
